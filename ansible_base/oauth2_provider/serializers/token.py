...
class OAuth2TokenSerializer(CommonModelSerializer):
    ...
    def _is_valid_scope(self, value):
        ...
        for word in words:
            ...
            if word not in get_scopes_backend().get_available_scopes():
                return False
        return True

    def validate_scope(self, value):
        if not self._is_valid_scope(value):
            raise ValidationError(_('Must be a simple space-separated string with allowed scopes {}.').format(get_scopes_backend().get_available_scopes()))
        return value
